...
export class AccountsRepository {
    ...
    async delete(address: string): Promise<boolean> {
        return this.runOnDB(async (connection) => {
            const res = await connection.query("DELETE FROM accounts.wallets WHERE address = $1", [address])
    ...
    async getList(from: number, to: number): Promise<Wallet[]> {
        const addresses: string[] = await this.runOnDB(async (connection) => {
            const res = await connection.query("SELECT * FROM accounts.wallets OFFSET $1 LIMIT $2", [from, to])
        ...
        for (let index = 0; index < addresses.length; index++) {
            ...
            const networks: Network[] = await this.runOnDB(async (connection) => {
                const res = await connection.query("SELECT * FROM accounts.wallet_networks WHERE address = $1", [address])
                return res.rows
                    .map(item => Network[item.network])
    ...
    async attachNetwork(address: string, network: Network): Promise<boolean> {
        return this.runOnDB(async (connection) => {
            const res = await connection.query("INSERT INTO accounts.wallet_networks(address, network) VALUES ($1, $2) RETURNING *", [address, network.toString()])
            return res.rowCount > 0
        })
    }
    ...
    async detachNetwork(address: string, network: Network): Promise<boolean> {
        return this.runOnDB(async (connection) => {
            const res = await connection.query("DELETE FROM accounts.wallet_networks WHERE address = $1 AND network = $2", [address, network.toString()])
            return res.rows.length == 0
        })
    }
